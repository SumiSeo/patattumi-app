...
const createCommentKorea = async (
  post_id: number,
  content: string,
  token: string
): Promise<PostResponse> => {
  try {
    const response = await fetch(`${API_URL}/comments/korea/${post_id}`, {
      method: "POST",
      headers: {
    ...
    console.error("Failed to create Comment in Korea:", error.message);
    throw error;
  }
};

export default createCommentKorea;
